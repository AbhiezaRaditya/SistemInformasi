<div>
    ...
    >
            ...
            <header class="fi-sidebar-header">
                @if ((! $hasTopbar) && $isSidebarCollapsibleOnDesktop)
                    <x-filament::icon-button
                        ...
                        :icon-alias="$isRtl ? [\Filament\View\PanelsIconAlias::SIDEBAR_EXPAND_BUTTON_RTL, \Filament\View\PanelsIconAlias::SIDEBAR_EXPAND_BUTTON] : \Filament\View\PanelsIconAlias::SIDEBAR_EXPAND_BUTTON"
                        icon-size="lg"
                ...
                @if ((! $hasTopbar) && ($isSidebarCollapsibleOnDesktop || $isSidebarFullyCollapsibleOnDesktop))
                    <x-filament::icon-button
                        ...
                        :icon-alias="$isRtl ? [\Filament\View\PanelsIconAlias::SIDEBAR_COLLAPSE_BUTTON_RTL, \Filament\View\PanelsIconAlias::SIDEBAR_COLLAPSE_BUTTON] : \Filament\View\PanelsIconAlias::SIDEBAR_COLLAPSE_BUTTON"
                        icon-size="lg"
        ...
        <nav class="fi-sidebar-nav">
            ...
            <script>
                ...
                if (collapsedGroups === null || collapsedGroups === 'null') {
                    localStorage.setItem('collapsedGroups', JSON.stringify(@js(collect($navigation)->filter(fn ($group): bool => $group->isCollapsed())->map(fn ($group): string => $group->getLabel())->values()->all())));
                }
                collapsedGroups = JSON.parse(localStorage.getItem('collapsedGroups'));
                document.querySelectorAll('.fi-sidebar-group').forEach((group) => {
                    if (!collapsedGroups.includes(group.dataset.groupLabel)) return;
        ...
        <div class="fi-sidebar-footer">
            ...
            {{-- Tombol Sign Out (POST, bukan GET — route logout cuma terima POST) --}}
            @if ($isAuthenticated)
                <form method="POST" action="{{ filament()->getLogoutUrl() }}" class="px-3 py-2">
                    @csrf
                    <x-filament::button
                        color="danger"
                        type="submit"
                        icon="heroicon-m-arrow-right-start-on-rectangle"
                        class="w-full justify-start custom-sidebar-signout-btn"
                    >
                        Sign Out
                    </x-filament::button>
                </form>
